...
namespace Math {
	class RoadMap {
		public:
			...
			virtual ~RoadMap();
			Math::Vector2<Config::Real> getTargetWay(const Math::Vector2<Config::Real> & worldCoordinates, const Config::Real & distanceAllowed) const;
			Math::Vector2<Config::Real> getPositionTowardTarget(Math::Vector2<Config::Real> & velocity, const Math::Vector2<Config::Real> & worldCoordinates, const double & agentSpeed, const Config::Real & dt) const;
